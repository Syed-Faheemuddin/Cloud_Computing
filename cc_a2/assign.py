...
def createRecord():
    ...
    u1 = UserController(surname, name)
    records = {"id": u1.get_id(),"surname": u1.get_surname(), "name": u1.get_name()}
    data = load_records(records)
    print(data)
    return f"ID: <b>{u1.get_id()}</b><br>Name: <b>{u1.get_name()}</b><br>Surname: <b>{u1.get_surname()}</b>"
    ...
    with open("records.json", "r") as f:
        ...
        for item in data['details']:
            if int(id) == int(item['id']):		
            	return f"Name: <b>{str(item['name'])}</b><br>Surname: <b>{str(item['surname'])}</b><br><br>"
